...
<style>
dl.order { margin-top:50px; }
dl.order dt { float:left; clear:left; color:#888; padding:5px; width:50px; border-bottom:1px solid #aaa; }
dl.order dd { float:left; padding:5px; width:300px; border-bottom:1px solid #aaa; }
</style>
...
<article>
	...
	<div class="main-contents">
		...
		<div class="info">
			...
			<dl class="order">
				...
				<dd>: ${sndGoodname}</dd>
				<dt>결재방법</dt>
				<dd>: ${payMethod==1 ? '신용카드' : payMethod==2 ? '휴대폰결재' : '무통장입금'}</dd>
				<dt>결재금액</dt>
				<dd>: <fmt:formatNumber>${sndAmount}</fmt:formatNumber>원</dd>
				<dt>이메일</dt>
				<dd>: ${sndEmail}</dd>
				<dt>연락처</dt>
				<dd>: ${sndMobile}</dd>
		...
		<form method="post" name="KSPayWeb" action = "/kspay/kspay_wh_result.jsp">
			<c:choose>
				<c:when test="${payMethod==1}">
					<input type='hidden' name='sndPaymethod' value='1000000000' size='15' maxlength='10'>
				</c:when>
				<c:when test="${payMethod==2}">
					<input type='hidden' name='sndPaymethod' value='0000010000' size='15' maxlength='10'>
				</c:when>
			</c:choose>
			<input type='hidden' name='sndStoreid' value='2062100213' size='15' maxlength='10'>
			<input type='hidden' name='sndOrdernumber' value='${sndOrdernumber}' size='30'>
			 <!--상품명 50Byte(한글 25자) 입니다. ' " ` 는 사용하실수 없습니다. 따옴표,쌍따옴표,백쿼테이션 -->
			<input type='hidden' name='sndGoodname' value='${sndGoodname}' size='30'>
			<input type='hidden' name='sndAmount' value='${sndAmount}' size='15' maxlength='9'>
			<input type='hidden' name='sndOrdername' value='${sndOrdername}' size='30'>
			<!--KSPAY에서 결제정보를 메일로 보내줍니다.(신용카드거래에만 해당)-->
			<input type='hidden' name='sndEmail' value='${sndEmail}' size='30'>
			<!--전화번호 value 값에 숫자만 넣게 해주시길 바랍니다. : '-' 가 들어가면 안됩니다.-->
			<input type='hidden' name='sndMobile' value='${sndMobile}' size='12' maxlength='12'>
	...
	<!----------------------------------------------- <Part 2. 추가설정항목(메뉴얼참조)>  ----------------------------------------------->
		...
		<input type=hidden	name=sndAdulttype value="0">  		<!--성인확인여부 (0:성인확인불필요, 1:성인확인필요) -->
